  ...
  if (confirmed) {
    return (
      <div className="confirmation-screen">
        ...
        <span className="confirmation-screen__order">Pedido #LM-2847</span>
      </div>
    );
  }
        ...
        <section className="payment-summary">
          ...
          <div className="payment-summary__card">
            {cartItems.map((item) => (
              <div key={item.id} className="payment-summary__row">
                <div className="payment-summary__item">
                  <img src={item.img} alt={item.name} className="payment-summary__img" />
                  <div>
            ...
            <div className="payment-summary__totals">
              ...
              <div className="summary-row"><span>Servicio (10%)</span><span>${(subtotal * 0.1).toFixed(2)}</span></div>
              <div className="summary-row summary-row--total">
                <span>Total</span>
                <span className="summary-row__amount">${total.toFixed(2)}</span>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="payment-methods">
          ...
          <div className="payment-methods__list">
            {PAYMENT_METHODS.map((m) => (
              <button
                key={m.id}
                className={`payment-method ${selectedMethod === m.id ? "payment-method--active" : ""}`}
